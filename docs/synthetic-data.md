...
This is the Rust-native train/generate path for reusable synthetic-data model artifacts. New artifacts use a lightweight rectified-flow / flow-matching trainer for numeric columns, while older DDPM-style artifacts remain readable.
...
- trains a rectified-flow / flow-matching model for numeric columns
- uses logit-normal time sampling during training
- uses Heun-style ODE sampling during generation
- saves a reusable model artifact as JSON
- generates synthetic numeric columns from the flow sampler for new artifacts, or from the legacy DDPM sampler for older artifacts
- bootstraps non-numeric columns from the reference dataset so the output keeps a usable schema
- trains a schema-preserving bootstrap artifact for datasets with no numeric columns
...
- the denoiser is intentionally linear and modular, so it can be replaced later by an MLP, gated network, or transformer
- latent-space diffusion, categorical diffusion, learned categorical decoders, neural denoisers, and formal differentially private training are not implemented yet
...
To validate the reusable trainer against the downloaded local datasets:

```powershell
./scripts/test-diffusion-trainer-downloaded-datasets.ps1
```

The script trains one model per downloaded top-level dataset, generates a small synthetic JSON sample from each trained model, and writes reports under `datasets\converted\diffusion-trainer-validation\`.

For the optional large performance corpus in `datasets\perf`, run:

```powershell
./scripts/test-diffusion-trainer-downloaded-datasets.ps1 -IncludePerf -Rows 4 -MaxTrainRows 64 -ReferenceMaxRows 64
```
