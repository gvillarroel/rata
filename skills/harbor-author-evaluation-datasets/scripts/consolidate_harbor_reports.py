...
def parse_iso_timestamp(value: Any, label: str, *, require_timezone: bool) -> tuple[str, datetime]:
...
def optional_number(
    ...
    *,
    ...
    if value is None:
        return None
    return float(require_number(value, label, minimum=minimum, maximum=maximum))
...
def read_report(path: Path, input_index: int) -> dict[str, Any]:
    ...
    if size > MAX_INPUT_BYTES:
        raise ReportError(f"report exceeds the {MAX_INPUT_BYTES}-byte input limit: {source}")
    ...
    if report.get("schemaVersion") != 1 or report.get("source") != "harbor":
        raise ReportError(f"report[{input_index}] must be a schemaVersion 1 Harbor final-report.json")
    ...
    comparison = require_object(report.get("comparison", {}), f"report[{input_index}].comparison")
...
def trial_metric(trials: list[Any], field: str, label: str) -> dict[str, Any] | None:
    values: list[float] = []
    for index, raw_trial in enumerate(trials):
        ...
        if len(present) > 1:
            ...
            if len(values_by_alias) != 1:
                raise ReportError(f"{label}.trials[{index}].tokens has conflicting aliases for {field}")
...
def parse_wall_seconds(started: Any, finished: Any, label: str) -> float | None:
    ...
    try:
        ...
        if start_is_aware != finish_is_aware:
            raise ValueError("startedAt and finishedAt must either both include a timezone or both omit it")
...
def normalize_job(source: dict[str, Any], raw_job: Any, job_index: int) -> dict[str, Any]:
    ...
    if passed + verifier_failed + errored != completed:
        raise ReportError(f"{label} pass, verifier-failure, and error counts must sum to completed trials")
    ...
    reward = normalize_metric_summary(summary.get("reward"), f"{label}.summary.reward", nonnegative=False)
    ...
    cost = normalize_metric_summary(summary.get("costUsd"), f"{label}.summary.costUsd", nonnegative=True)
    for metric_name, metric in (
        ...
        if metric is not None and metric["count"] > completed:
            raise ReportError(f"{label}.summary.{metric_name}.count exceeds completedTrials")
    ...
    token_metrics = {field: trial_metric(trials, field, label) for field in ("input", "cache", "output", "reasoning")}
    if (
        token_metrics["input"]
        and token_metrics["cache"]
        and token_metrics["input"]["complete"]
        and token_metrics["cache"]["complete"]
        and token_metrics["cache"]["total"] > token_metrics["input"]["total"] + 1e-9
    ):
    ...
    if (
        ...
        derived_total = token_metrics["input"]["total"] + token_metrics["output"]["total"]
        if not math.isclose(total_tokens["total"], derived_total, rel_tol=1e-6, abs_tol=1e-6):
            raise ReportError(f"{label} total tokens must equal input plus output; cache is already included in input")
    ...
    run_id = hashlib.sha256(f"{source['sha256']}\0{job_identity}\0{original_label}".encode()).hexdigest()[:16]
    ...
    total_tokens_coverage_complete = bool(total_tokens and total_tokens["count"] == completed)
    cost_coverage_complete = bool(cost and cost["count"] == completed)
    agent_time_coverage_complete = bool(latency_ms and latency_ms["count"] == completed)
    return {
        "runId": run_id,
        ...
        "tasksPerMinute": completed * 60 / wall_seconds if wall_seconds and wall_seconds > 0 else None,
        "tokensPerTrial": total_token_value / completed if total_tokens_coverage_complete and completed else None,
        "costPerTrialUsd": total_cost / completed if cost_coverage_complete and completed else None,
        "costPerPassUsd": total_cost / passed if cost_coverage_complete and passed else None,
        "agentSecondsPerTrial": agent_seconds / completed if agent_time_coverage_complete and completed else None,
    }
...
def assign_display_labels(runs: list[dict[str, Any]]) -> None:
    ...
    for index, (run, display) in enumerate(zip(runs, preliminary, strict=True), start=1):
        run["label"] = display if display_counts[display] == 1 else f"{display} [{index}]"


def choose_baseline(runs: list[dict[str, Any]], requested: str | None) -> dict[str, Any]:
    if requested is None:
        return runs[0]
    matches = [run for run in runs if requested in {run["runId"], run["label"], run["originalLabel"]}]
    if len(matches) != 1:
        raise ReportError(
...
def build_report(
    ...
    *,
    ...
    runs = [normalize_job(source, raw_job, index) for source in sources for index, raw_job in enumerate(source["jobs"])]
    ...
    if generated_at is None:
        resolved_generated_at = max(sources, key=lambda source: source["generatedAtValue"])["generatedAt"]
    else:
        resolved_generated_at, _ = parse_iso_timestamp(generated_at, "--generated-at", require_timezone=True)
    source_records = [
        {
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    format_number(run["wallTimeSeconds"], 1) + " s" if run["wallTimeSeconds"] is not None else "n/a",
...
def render_quality_svg(report: dict[str, Any]) -> str:
    ...
    for panel_x, maximum, percentage in ((left_x, 1.0, True), (right_x, reward_ceiling, False)):
        ...
        for tick in range(5):
            ...
            body.append(
                f'<line x1="{x:.1f}" y1="{panel_y + 60}" x2="{x:.1f}" y2="{panel_y + panel_h - 30}" class="grid"/>'
            )
            body.append(
                f'<text x="{x:.1f}" y="{panel_y + panel_h - 12}" text-anchor="middle" class="small">{esc(label)}</text>'
            )
    for index, run in enumerate(runs):
        ...
        for panel_x in (left_x, right_x):
            body.append(
                f'<text x="{panel_x + 28}" y="{y + 18}" class="label">{esc(baseline_mark + truncate(run["label"], 20))}</text>'
            )
        ...
        body.append(
            f'<text x="{left_x + panel_w - 24}" y="{y + 18}" text-anchor="end" class="value">{format_percent(run["passRate"])}</text>'
        )
        ...
        if reward is not None:
            body.append(
                f'<rect x="{right_x + 190}" y="{y}" width="{reward_width:.1f}" height="24" rx="12" fill="{color}"/>'
            )
        ...
        body.append(
            f'<text x="{right_x + panel_w - 24}" y="{y + 18}" text-anchor="end" class="value">{esc(reward_label)}</text>'
        )
        if run["erroredTrials"]:
            body.append(f'<circle cx="{left_x + 171}" cy="{y + 12}" r="9" fill="#FB7185"/>')
            body.append(
                f'<text x="{left_x + 171}" y="{y + 16}" text-anchor="middle" font-size="10" fill="#111827">{run["erroredTrials"]}</text>'
            )
    body.append(
        f'<text x="56" y="{height - 34}" class="small">A red badge is the number of execution errors. A dot marks the selected baseline. Aggregate views do not replace native fairness checks.</text>'
    )
    return svg_document(
...
def render_resource_svg(report: dict[str, Any]) -> str:
    ...
    total_values = [run["tokens"]["total"]["total"] for run in runs if run["tokens"]["total"] is not None]
    cost_values = [run["costUsd"] for run in runs if run["costUsd"] is not None]
    time_values = [
        value for run in runs for value in (run["agentTimeSeconds"], run["wallTimeSeconds"]) if value is not None
    ]
    ...
    for index, run in enumerate(runs):
        ...
        if input_tokens is not None and output_tokens is not None:
            ...
            for value, color in ((fresh, "#38BDF8"), (cache, "#A78BFA"), (output_tokens, "#34D399")):
                ...
                if segment > 0:
                    body.append(
                        f'<rect x="{cursor:.1f}" y="{y}" width="{segment:.1f}" height="26" rx="5" fill="{color}"/>'
                    )
                    cursor += segment
        ...
        body.append(
            f'<text x="{token_x}" y="{y + 48}" class="small">total {esc(total_label)} · reasoning {esc(compact(reasoning))}</text>'
        )
        ...
        for offset, value, color, prefix in (
            ...
            body.append(
                f'<text x="{time_x - 20}" y="{y + offset + 13}" text-anchor="end" class="small">{prefix}</text>'
            )
            body.append(f'<rect x="{time_x}" y="{y + offset}" width="{time_w}" height="15" rx="7.5" fill="#1E293B"/>')
            if value is not None:
                bar_width = time_w * value / time_max
                body.append(
                    f'<rect x="{time_x}" y="{y + offset}" width="{bar_width:.1f}" height="15" rx="7.5" fill="{color}"/>'
                )
        ...
        body.append(
            f'<text x="{time_x}" y="{y + 62}" class="small">{esc(agent_label)} agent · {esc(compact(run["wallTimeSeconds"]))}s wall</text>'
        )
    body.append(
        f'<text x="56" y="{height - 32}" class="small">Token scale: {esc(compact(token_max))}. Cost scale: {esc(compact(cost_max, money=True))}. Time scale: {esc(compact(time_max))} seconds. Reasoning is supplemental and never silently added to total.</text>'
    )
    return svg_document(
...
def pareto_runs(points: list[tuple[dict[str, Any], float, float]]) -> set[str]:
    ...
    for run, cost, quality in points:
        dominated = any(
            other_cost <= cost and other_quality >= quality and (other_cost < cost or other_quality > quality)
...
def render_efficiency_svg(report: dict[str, Any]) -> str:
    ...
    points = [(run, run["costPerTrialUsd"], run["passRate"]) for run in runs if run["costPerTrialUsd"] is not None]
    ...
    for tick in range(5):
        ...
        body.append(
            f'<text x="{x:.1f}" y="{chart_y + chart_h + 26}" text-anchor="middle" class="small">{esc(compact(cost_max * tick / 4, money=True))}</text>'
        )
        body.append(f'<text x="{chart_x - 16}" y="{y + 4:.1f}" text-anchor="end" class="small">{tick * 25}%</text>')
    body.append(
        f'<text x="{chart_x + chart_w / 2}" y="{chart_y + chart_h + 58}" text-anchor="middle" class="small">cost per completed trial (USD) →</text>'
    )
    body.append(
        f'<text x="28" y="{chart_y + chart_h / 2}" transform="rotate(-90 28 {chart_y + chart_h / 2})" text-anchor="middle" class="small">pass rate →</text>'
    )
    frontier_points = sorted([point for point in points if point[0]["runId"] in frontier], key=lambda item: item[1])
    if len(frontier_points) > 1:
        ...
        body.append(
            f'<path d="{path_data}" fill="none" stroke="#FBBF24" stroke-width="2" stroke-dasharray="7 7" opacity="0.8"/>'
        )
    ...
    for index, (run, cost, quality) in enumerate(points):
        ...
        if run["runId"] in frontier:
            body.append(
                f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{radius + 7:.1f}" fill="none" stroke="#FBBF24" stroke-width="2"/>'
            )
        if run["runId"] == report["baselineRunId"]:
            body.append(
                f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{radius + 12:.1f}" fill="none" stroke="#E2E8F0" stroke-width="2" stroke-dasharray="4 5"/>'
            )
        body.append(
            f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{radius:.1f}" fill="{color}" fill-opacity="0.86" stroke="#F8FAFC" stroke-width="1.5"/>'
        )
        label_y = y - radius - 18 if index % 2 == 0 else y + radius + 28
        body.append(
            f'<text x="{x:.1f}" y="{label_y:.1f}" text-anchor="middle" class="label">{esc(truncate(run["label"], 22))}</text>'
        )
    if not points:
        body.append(
            f'<text x="{chart_x + chart_w / 2}" y="{chart_y + chart_h / 2}" text-anchor="middle" class="subtitle">No complete cost metric is available for a frontier.</text>'
        )
    for index, run in enumerate(runs):
        y = 274 + index * 52
        color = PALETTE[index % len(PALETTE)]
        ...
        body.append(
            f'<text x="1126" y="{y + 19}" class="small">{esc(compact(run["tokensPerTrial"]))} tok/trial · {esc(compact(run["costPerPassUsd"], money=True))}/pass · {esc(compact(run["agentSecondsPerTrial"]))}s agent/trial</text>'
        )
    footer_y = height - 30
    body.append(
        f'<text x="56" y="{footer_y}" class="small">Gold rings mark non-dominated aggregate points; dashed white marks the baseline. Missing or partial cost values are omitted from the frontier; partial resource totals never drive efficiency deltas.</text>'
    )
    return svg_document(
...
def write_outputs(output: Path, payloads: dict[str, bytes], overwrite: bool) -> None:
    ...
    if existing and not overwrite:
        raise ReportError(
            "refusing to overwrite existing report artifacts: " + ", ".join(path.name for path in existing)
        )
