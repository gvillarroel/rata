...
def require_exact_keys(value: dict[str, Any], required: set[str], label: str, optional: set[str] | None = None) -> None:
...
def require_id(value: Any, label: str) -> str:
    if not isinstance(value, str) or not ID_PATTERN.fullmatch(value):
        raise PlanError(f"{label} must be a lowercase identifier using letters, digits, '.', '_', or '-'")
    return value
...
def normalize_minimum_map(value: Any, label: str, *, allowed_keys: set[str] | None = None) -> dict[str, int]:
...
def normalize_coverage_requirements(value: Any, split_weights: dict[str, int]) -> dict[str, Any]:
    ...
    if set(raw_requirements) != set(split_weights):
        ...
        raise PlanError("coverageRequirements must match declared splits: " + "; ".join(details))
    ...
    for split in split_weights:
        ...
        normalized[split] = {
            "minimumFamilies": require_positive_integer(requirement["minimumFamilies"], f"{label}.minimumFamilies"),
            "minimumTasks": require_positive_integer(requirement["minimumTasks"], f"{label}.minimumTasks"),
            ...
            "strata": {
                dimension: normalize_minimum_map(raw_strata[dimension], f"{label}.strata.{dimension}")
...
def validate_coverage_feasibility(families: list[dict[str, Any]], coverage: dict[str, Any]) -> None:
    ...
    available_modes = Counter(case["responseMode"] for family in families for case in family["cases"])
    ...
    for mode, minimum in sorted(required_modes.items()):
        if minimum > available_modes[mode]:
            raise PlanError(f"coverage requires {minimum} {mode} tasks but only {available_modes[mode]} exist")
...
def normalize_blueprint(value: Any) -> dict[str, Any]:
    ...
    for family_index, raw_family in enumerate(family_values):
        ...
        require_exact_keys(raw_strata, set(STRATA_DIMENSIONS), f"{label}.strata")
        strata = {
            dimension: require_id(raw_strata[dimension], f"{label}.strata.{dimension}")
            for dimension in STRATA_DIMENSIONS
        }
        ...
        for shared_id, owners, kind in (
            ...
            if owner is not None and owner != family_id:
                raise PlanError(f"{kind} {shared_id} spans families {owner} and {family_id}; merge the semantic family")
        ...
        for case_index, raw_case in enumerate(case_values):
            ...
            response_mode = require_text(case["responseMode"], f"{case_label}.responseMode")
            if response_mode not in RESPONSE_MODES:
                raise PlanError(f"{case_label}.responseMode must be one of: {', '.join(sorted(RESPONSE_MODES))}")
            ...
            for axis, raw_options in sorted(raw_axes.items()):
                ...
                options = require_list(raw_options, f"{case_label}.variantAxes.{axis_id}")
                if not options:
                    raise PlanError(f"{case_label}.variantAxes.{axis_id} must not be empty")
                normalized_options = sorted(
                    require_text(option, f"{case_label}.variantAxes.{axis_id} option") for option in options
                )
                if len(normalized_options) != len(set(normalized_options)):
                    raise PlanError(f"{case_label}.variantAxes.{axis_id} contains duplicate options")
    ...
    if len(normalized_families) < len(split_weights):
        raise PlanError("the blueprint needs at least one semantic family for every declared split")
    normalized_families.sort(key=lambda item: item["familyId"])
    coverage = normalize_coverage_requirements(blueprint["coverageRequirements"], split_weights)
...
def normalize_seeds(value: Any, declared_splits: set[str]) -> dict[str, Any]:
    ...
    require_exact_keys(seeds, {"schemaVersion", "partitionSeed", "variantSeeds"}, "seeds")
    ...
    if any(not SEED_PATTERN.fullmatch(seed) for seed in all_seeds):
        raise PlanError(
            "every seed must be exactly 64 lowercase hexadecimal characters; use a cryptographically secure generator"
        )
...
def assign_families(
    ...
    def coverage_gain(family: dict[str, Any], split: str) -> float:
        ...
        for dimension in STRATA_DIMENSIONS:
            ...
            if minimum:
                shortfall = max(0, minimum - strata_counts[dimension][value_id][split])
                gain += min(1, shortfall) / minimum
        return gain

    def allocation_key(family: dict[str, Any], split: str) -> tuple[float, float, float, float, str, str]:
        weight = split_weights[split]
        profile = response_profile(family)
        strata_pressure = sum(
            (strata_counts[dimension][family["strata"][dimension]][split] + 1) / weight
            for dimension in STRATA_DIMENSIONS
        )
        response_pressure = sum((response_counts[mode][split] + count) / weight for mode, count in profile.items())
        return (
    ...
    for family in remaining:
        selected_split = min(split_weights, key=lambda split: allocation_key(family, split))
        ...
        for case in family["cases"]:
            ...
            for axis, options in case["variantAxes"].items():
                selected[task_id][axis] = max(
                    options,
                    key=lambda option: (
                        keyed_digest(seed, "axis-choice", split, axis, task_id, option),
                        option,
                    ),
                )
    return selected
...
def validate_observed_coverage(
    ...
    for family in families:
        for dimension in STRATA_DIMENSIONS:
            strata_counts[dimension][family["strata"][dimension]][family["split"]] += 1

    failures: list[str] = []
    for split, requirement in coverage.items():
        if family_counts[split] < requirement["minimumFamilies"]:
            failures.append(f"{split} has {family_counts[split]} families; requires {requirement['minimumFamilies']}")
        if task_counts[split] < requirement["minimumTasks"]:
            failures.append(f"{split} has {task_counts[split]} tasks; requires {requirement['minimumTasks']}")
        for mode, minimum in requirement["responseModes"].items():
            observed = response_counts[mode][split]
            if observed < minimum:
                failures.append(f"{split} has {observed} {mode} tasks; requires {minimum}")
        for dimension in STRATA_DIMENSIONS:
            for value_id, minimum in requirement["strata"][dimension].items():
                observed = strata_counts[dimension][value_id][split]
                if observed < minimum:
                    failures.append(f"{split} has {observed} families with {dimension}={value_id}; requires {minimum}")
    if failures:
        raise PlanError("planned split coverage is unmet: " + "; ".join(failures))


def build_plan(blueprint: dict[str, Any], seeds: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    families = blueprint["families"]
    split_weights = blueprint["splitWeights"]
    coverage = blueprint["coverageRequirements"]
    assignments = assign_families(families, split_weights, coverage, seeds["partitionSeed"])
    ...
    for family in families:
        ...
        for case in family["cases"]:
            ...
            variant_id = digest_value({"split": split, "taskId": task_id, "variantAxes": selected_axes})[:16]
            task_records.append(
                {
                    "taskId": task_id,
...
def build_summary_core(plan_core: dict[str, Any], plan_digest: str) -> dict[str, Any]:
    ...
    for split in plan_core["splitWeights"]:
        ...
        axis_counts = Counter(axis for task in tasks for axis in task["variantAxes"])
        split_counts[split] = {
            "families": len(families_by_split[split]),
            "tasks": len(tasks),
            "strata": {
                dimension: len({family["strata"][dimension] for family in families_by_split[split]})
                for dimension in STRATA_DIMENSIONS
            },
...
def validate_generated_plan(plan: Any) -> dict[str, Any]:
    ...
    coverage = normalize_coverage_requirements(value["coverageRequirements"], split_weights)
    ...
    if not isinstance(value["blueprintSha256"], str) or not SHA256_PATTERN.fullmatch(value["blueprintSha256"]):
        raise PlanError("plan.private.json.blueprintSha256 must be a SHA-256 digest")

    commitments = require_object(value["seedCommitments"], "plan.private.json.seedCommitments")
    require_exact_keys(commitments, {"partition", "variants"}, "plan.private.json.seedCommitments")
    if not isinstance(commitments["partition"], str) or not SHA256_PATTERN.fullmatch(commitments["partition"]):
        raise PlanError("partition seed commitment must be a SHA-256 digest")
    variant_commitments = require_object(commitments["variants"], "plan.private.json.seedCommitments.variants")
    if set(variant_commitments) != set(split_weights):
        raise PlanError("variant seed commitments must match declared splits")
    if any(
        not isinstance(digest, str) or not SHA256_PATTERN.fullmatch(digest) for digest in variant_commitments.values()
    ):
    ...
    if set(declared_task_owners) != {
        require_id(task.get("taskId"), "generated taskId") for task in task_values if isinstance(task, dict)
    }:
    ...
    for index, raw_task in enumerate(task_values):
        ...
        expected_variant_id = digest_value({"split": task["split"], "taskId": task_id, "variantAxes": axes})[:16]
...
def command_verify(args: argparse.Namespace) -> int:
    ...
    if summary != expected_summary:
        raise PlanError("summary.redacted.json is not the exact redacted projection of the private plan")
    ...
    if args.blueprint is not None and args.seeds is not None:
        ...
        if plan_bytes != canonical_bytes(reproduced_plan):
            raise PlanError("plan.private.json does not reproduce from the supplied blueprint and seeds")
        if summary_bytes != canonical_bytes(reproduced_summary):
            raise PlanError("summary.redacted.json does not reproduce from the supplied blueprint and seeds")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    plan_parser = subparsers.add_parser("plan", help="assign semantic families and stable per-task seeded variants")
    ...
    verify_parser.add_argument("--blueprint", type=Path, help="private blueprint; requires --seeds")
    verify_parser.add_argument("--seeds", type=Path, help="private seed file; requires --blueprint")
